...
bool Logger::log(LogLevel level, std::string_view str) noexcept
{
	...
	return this->buffer.Enqueue(buffer, static_cast<int>(written)) == static_cast<int>(written);
}
...
bool Logger::log(LogLevel level, const char* fmtstr, ...) noexcept
{
	...
	sprintf_ret = sprintf_s(buffer, sizeof(buffer), "[%s] ", level_prefix);
	if (sprintf_ret < 0)
		return false;
...
{
	while (this->buffer.GetUseSize())
	{
		int         count;
		const char* ptr;
		std::size_t bytes_written;

		count = this->buffer.DirectDequeueSize();
		if (!count)
			break;
		ptr = this->buffer.GetFrontBufferPtr();
		bytes_written = this->sink.write(ptr, static_cast<std::size_t>(count));
		if (!bytes_written)
			return false;
		this->buffer.MoveFront(static_cast<int>(bytes_written));
	}
...
Logger::Logger(void) noexcept
	: buffer(static_cast<int>(BUFFER_SIZE))
...
bool Logger::Sink::open(void) noexcept
{
	...
	case Logger::SinkTarget::FILE:
		...
		if (this->backend == Logger::SinkBackend::STDIO)
		{
			fopen_s(&this->handle.fp, this->filepath.string().c_str(), "ab");
			return this->handle.fp != nullptr;
		}
		else if (this->backend == Logger::SinkBackend::POSIX)
		{
#if defined(_WIN32) || defined(_WIN64)
			_sopen_s(&this->handle.fd, this->filepath.string().c_str(), LOGGER_OPEN_FLAG, _SH_DENYNO, LOGGER_OPEN_MODE);
#else
			this->handle.fd = ::open(this->filepath.string().c_str(), LOGGER_OPEN_FLAG, LOGGER_OPEN_MODE);
#endif
